    ...
    def itemCF(self, urls):
        ...
        for i in urls:
            ...
            for j, wj in sorted(self.W[i].items(), key=itemgetter(1), reverse=True)[0:self.k]:
                # 如果已经包含了物品j，跳过
                bookj = self.mongodb.search_book_by_url(j)
                if j in urls:
                    continue
                # 如果物品j是无效书籍，跳过
                ...
                rank[j] += interest * wj
                print '喜欢节目：'+ book['bookName']+'     和   '+bookj['bookName']+'   的相似度为'+ str(self.W[i][j])+'，兴趣度'+str(rank[j])
            print '-------------------------------------------------------------------------------------'
        # 按照兴趣度排序，返回推荐书籍的urls
...
if __name__ == '__main__':
    ...
    urls = []
    urls.append('https://book.douban.com/subject/1140457/')
    urls.append('https://book.douban.com/subject/1110941/')
    for i in urls:
        variety= recommend.mongodb.search_book_by_url(i)
        print variety['bookName']
    print '-----------------------------------------------'
    print '根据用户收藏的综艺节目，推荐的列表：'
    for i in recommend.itemCF(urls):
        variety = recommend.mongodb.search_book_by_url(i)
        interest = variety['bookName']
        print interest
    print 'all down!'
